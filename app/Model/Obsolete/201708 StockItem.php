			...
			return $this->find('all', array(
				...
				'fields'=>array(
					...
					'(SELECT SUM(`stock_items`.`remaining_quantity`) 
						FROM stock_items 
						right outer JOIN `products` ON (`stock_items`.`product_id` = `products`.`id`) 
						WHERE production_result_code_id="3" 
							AND stock_items.product_id=Product.id
							AND stock_items.raw_material_id = RawMaterial.id
					) AS Remaining_C',
					'(SELECT SUM(`stock_items`.`remaining_quantity`*`stock_items`.`product_unit_price`) 
				...
				'conditions' => array('Product.product_type_id'=> $productTypeIds),
				'contain' => array(
					'ProductionResultCode'=>array(
						'fields'=>array('code')
					),
      ...
      'StockItem.product_id'=> $productIds,
		...
		switch ($productTypeId){
			case PRODUCT_TYPE_PREFORMA:
				...
				//20171010 NEEDS TO BE OVERWRITTEN TO SORT BY PRODUCT.NAME
        ...
        //$printConditions[]=array('StockItem.product_id'=>12);
        //$printConditions[]=array('StockItem.raw_material_id'=>24);
        //$stockItemIdList=$this->find('all',array(
        //  'fields'=>'StockItem.id',
        //  'conditions'=>$printConditions,
        //));
        //foreach ($stockItemIdList as $stockItem){
        ...
        //}
				...
				$bottleCount =$this->find('count', array(
					...
					*/	
					...
					'group'=>'Product.id,RawMaterial.name',
				));
