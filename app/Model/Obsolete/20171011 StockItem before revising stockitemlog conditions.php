...
class StockItem extends AppModel {
    ...
    $productIds=$productModel->find('list',array(
      ...
      'conditions'=>array(
        ...
        //echo "preforma count is ".count($preformas)."<br/>";
				//20171010 TODO NEEDS TO BE OVERWRITTEN TO SORT BY PRODUCT.NAME
        // the $preformas variable now holds 
				// now overwrite based on StockItemLogs
				for ($i=0;$i<count($preformas);$i++){
					$stockItemConditions=array('StockItem.product_id'=>$preformas[$i]['Product']['id']);
					if ($warehouseId>0){
						$stockItemConditions[]=array('StockItem.warehouse_id'=>$warehouseId,);
					}
          // 20171010 TODO this needs to be reduced because it returns all stockitems, whether they have a remaining quantity of zero or not on the date
          // if the date is the current date, it would be enough to look at remaining quantity, which would be hyperfast
          // else an optimized sql query could be considered based on conditions to only retain the ones with positive remaining stock on inventory date
          // if not the original list will always be a serious overshoot
            ...
            'contain'=>array(
              'StockItemLog'=>array(
					...
					echo "count of all stockitems is ".count($allStockItems)."<br/>";
        ...
        //20171010 TODO NEEDS TO BE OVERWRITTEN TO SORT BY RAW MATERIAL NAME, THEN THE PRODUCT NAME
        // AS BEFORE WITH RECURSIVE 0 THE ORDER WAS
        // 'order'=>'ISNULL(RawMaterial.name), RawMaterial.name ASC, Product.name ASC',
        
        
				// now overwrite based on StockItemLogs
				for ($i=0;$i<count($productos);$i++){
          //if ($productos[$i]['RawMaterial']['id']==24&&$productos[$i]['Product']['id']==12){
          //  pr($productos[$i]);
          //}
            ...
            for ($productionresultcode=1;$productionresultcode<4;$productionresultcode++){
              ...
              //if ($boolQuantitiesAtCurrentDate){
              //  $stockItemConditions[]=array('StockItem.remaining_quantity >'=>0,);
              //}
              $allStockItems=$this->find('all',array(
                'fields'=>array('StockItem.id'),
                'conditions'=>$stockItemConditions,
              ...
              switch ($productionresultcode){
				...
				//pr($products);
				//echo "these were the products delivered by getInventoryItems<br/>";
				break;
			case PRODUCT_TYPE_CAP:
				...
				//20171010 TODO SORT BY PRODUCT NAME
				// now overwrite based on StockItemLogs
				for ($i=0;$i<count($tapones);$i++){
					$stockItemConditions=array('StockItem.product_id'=>$tapones[$i]['Product']['id']);
					if ($warehouseId>0){
						$stockItemConditions[]=array('StockItem.warehouse_id'=>$warehouseId,);
					}
					$allStockItems=$this->find('all',array(
  ...
  function getInventoryFinishedProduct($finishedProductId,$rawMaterialId,$inventoryDate,$warehouseId=0){
	...
	function getFinishedMaterialsForSale($productid=null,$resultcodeid,$quantityneeded=0,$rawmaterialid=0,$saledate, $warehouseId=0, $orderby="ASC"){
