...
<script>
  ...
  //jsProviderNatures=<?php  //echo json_encode($providerNatures); ?>;
  
  jsProductUnits=<?php  echo json_encode($productUnits); ?>;
  jsProductPackagingUnits=<?php  echo json_encode($productPackagingUnits); ?>;
  jsProductDefaultCosts=<?php  echo json_encode($productDefaultCosts); ?>;
  ...
  $('body').on('change','#PurchaseOrderProviderId',function(){	
    ...
    //$('#PurchaseOrderBoolPackaging').prop('checked',(jsProviderNatures[providerId] == <?php echo PROVIDER_NATURE_CONSUMABLE; ?>)?true:false);
		setDueDate();
		setOrderType();
	});  
	function setDueDate(){
   ...
   $('body').on('change','#PurchaseOrderPurchaseOrderStateId',function(){	
    var purchaseOrderState=$(this).val();
    if (purchaseOrderState < <?php echo PURCHASE_ORDER_STATE_AUTHORIZED; ?>){
      
     $('#PurchaseOrderBoolAuthorized').prop('disabled',false); $('#PurchaseOrderBoolAuthorized').prop('checked',false);
     $('#PurchaseOrderBoolAuthorized').prop('disabled',true); 
    }
    else {
      $('#PurchaseOrderBoolAuthorized').prop('disabled',false); $('#PurchaseOrderBoolAuthorized').prop('checked',true);
      $('#PurchaseOrderBoolAuthorized').prop('disabled',true);
    }
  });
  ...
  $('body').on('change','.productid',function(){
    ...
    var productId=thisRow.find('td.productid div select').val();
    if (productId > 0){
      var packagingUnit=jsProductPackagingUnits[productId];
      thisRow.find('.productpackagingunit').val(packagingUnit)
      thisRow.find('.productquantity div input').val(packagingUnit)
      calculatePackagingUnits(thisRow);
      thisRow.find('.productunit div select').val(jsProductUnits[productId])  
      thisRow.find('.productunitcost div input').val(jsProductDefaultCosts[productId])
    }
    else {
      thisRow.find('.productpackagingunit').val(0)
      thisRow.find('.productquantity div input').val(0)
      calculatePackagingUnits(thisRow);
      thisRow.find('.productunit div select').val(0)  
      thisRow.find('.productunitcost div input').val(0)
    }
		calculateRow(thisRow.attr('row'));
    calculateTotal();
	});
  ...
  function resetState(){
    $('#stateAlert').removeClass('d-none');
    $('#PurchaseOrderPurchaseOrderStateId').val(<?php echo PURCHASE_ORDER_STATE_AWAITING_AUTHORIZATION; ?>);
    
    $('#purchaseOrderState').html('<?php echo $purchaseOrderStates[PURCHASE_ORDER_STATE_AWAITING_AUTHORIZATION]; ?>');
    $('#PurchaseOrderBoolAuthorized').prop('checked',false);
    
    var purchaseOrderCode=$('#PurchaseOrderPurchaseOrderCode').val();
    newPurchaseOrderCode=addEditionToPurchaseOrderCode(purchaseOrderCode);
    $('#PurchaseOrderPurchaseOrderCode').val(newPurchaseOrderCode);
    $('#purchaseOrderTitleCode').html(newPurchaseOrderCode);
  }
  
  function addEditionToPurchaseOrderCode(purchaseOrderCode){
    var newEditionEnding="_E1";
    var finalComponentCode=purchaseOrderCode.substr(purchaseOrderCode.lastIndexOf("_")+1);
    if (finalComponentCode.startsWith("E")){
      var editionNumber=parseInt(purchaseOrderCode.substr(purchaseOrderCode.lastIndexOf("_")+2));
      if (isNaN(editionNumber)){
        alert('No se podía actualizar el número de edición porque '+editionNumber+' no es un número')
      }
      else {
        newEditionEnding="_E"+(editionNumber+1)
      }
      return purchaseOrderCode.substr(0,purchaseOrderCode.lastIndexOf("_"))+newEditionEnding
    }
    return purchaseOrderCode+newEditionEnding
  }
  ...
  function roundToTwo(num) {    
    ...
    if (<?php echo $this->request->data['PurchaseOrder']['purchase_order_state_id']; ?> < <?php echo PURCHASE_ORDER_STATE_RECEIVED_COMPLETELY; ?>){
      // $('#PurchaseOrderPurchaseOrderStateId option[value="<?php //echo PURCHASE_ORDER_STATE_RECEIVED_PARTIALLY; ?>"]').prop('disabled',true); 
      $('#PurchaseOrderPurchaseOrderStateId option[value="<?php echo PURCHASE_ORDER_STATE_RECEIVED_COMPLETELY; ?>"]').prop('disabled',true);
    }
...
<?php 
  ...
  echo "</div>";
	...
	echo '<fieldset id="mainform">';
		echo '<legend>'.__('Editar Orden de Compra').'<span id ="purchaseOrderTitleCode">'.$this->request->data['PurchaseOrder']['purchase_order_code'].'</span></legend>';
    echo '<h3 id="PurchaseOrderPlant">'.$plants[$warehousePlants[$this->request->data['PurchaseOrder']['warehouse_id']]].'</h3>';
		echo '<p id="stateAlert" style="background-color:orange;font-size:1.3em;font-weight:bold" class="d-none">Al editar los productos de una orden de compra, se añade "E" al código más el número de ediciones, y se restablece el estado a no autorizado.  Vuelve a autorizar y confirmar el estado de orden de compra.</p>';
		echo "<div class='container-fluid'>";
			echo "<div class='row'>";
				echo "<div class='col-sm-6'>";
          echo $this->WarehouseFilter->displayWarehouseFilter($warehouses, $userRoleId,$this->request->data['PurchaseOrder']['warehouse_id']);
          
					echo $this->Form->input('purchase_order_date',['dateFormat'=>'DMY','minYear'=>2014,'maxYear'=>date('Y')+1]);
					echo $this->Form->input('purchase_order_code',['readonly'=>true]);
          
          echo $this->Form->input('provider_id',['empty'=>['0'=>'Seleccione Proveedor']]);
          //echo $this->Form->input('bool_packaging',['label'=>'Compra Suministros?']);
					
          if ($userRoleId == ROLE_ADMIN){
            switch ($this->request->data['PurchaseOrder']['purchase_order_state_id']){
              case PURCHASE_ORDER_STATE_AWAITING_AUTHORIZATION:
              case PURCHASE_ORDER_STATE_AUTHORIZED:
              case PURCHASE_ORDER_STATE_CONFIRMED_WITH_CLIENT:
                echo $this->Form->input('purchase_order_state_id',['label'=>'Estado']);
                break;
              //case PURCHASE_ORDER_STATE_RECEIVED_PARTIALLY:
              case PURCHASE_ORDER_STATE_RECEIVED_COMPLETELY:
              default:
                echo $this->Form->input('purchase_order_state_id',['label'=>'Estado','class'=>'fixed']);
            
            }
          }
          else {
            echo $this->Form->input('purchase_order_state_id',['type'=>'hidden']);
          }
          echo '<p>El estado de la orden de compra es <span id="purchaseOrderState" style="font-weight:700;font-size:1.2em;">'.$purchaseOrderStates[$this->request->data['PurchaseOrder']['purchase_order_state_id']].'</span></p>';
          ...
          echo "<div id='divDueDate'>";
            echo $this->Form->input('due_date',['type'=>'date','label'=>__('Fecha de Vencimiento'),'dateFormat'=>'DMY','minYear'=>2014,'maxYear'=>(date('Y')+1)]);
          echo "</div>";
          //if ($userRoleId === ROLE_ADMIN){
            echo $this->Form->input('bool_authorized',['type'=>'checkbox','label'=>'Autorizada','disabled'=>true]);
          //}
				echo "</div>";
          ...
          if ($this->request->data['PurchaseOrder']['purchase_order_state_id']< PURCHASE_ORDER_STATE_RECEIVED_COMPLETELY){
            echo $this->Form->Submit(__('Guardar'),['class'=>'save','name'=>'save']);
          }
          else {
            echo '<p class="warning">No se puede editar esta orden de compra porque ya ha estado recibido</p>';
          }
                ...
                echo '<th>Und</th>';
						...
						for ($pop=0;$pop<count($requestProducts);$pop++){
							...
							echo "<tr row='".$pop."'>";
								echo "<td class='productid width100'>";
                ...
                echo "<td class='productquantity amount'>".$this->Form->input('PurchaseOrderProduct.'.$pop.'.product_quantity',['label'=>false,'type'=>'decimal','value'=>$requestProducts[$pop]['PurchaseOrderProduct']['product_quantity'],'required'=>false])."</td>";
								echo "<td class='productunit'>".$this->Form->input('PurchaseOrderProduct.'.$pop.'.unit_id',['label'=>false,'value'=>$requestProducts[$pop]['PurchaseOrderProduct']['unit_id'],'empty'=>[0=>'-- Unidad --'],])."</td>";
                ...
                echo "<td class='productunit'>".$this->Form->input('PurchaseOrderProduct.'.$pop.'.unit_id',['label'=>false,'value'=>(empty($requestProducts[$pop]['PurchaseOrderProduct']['unit_id'])?0:$requestProducts[$pop]['PurchaseOrderProduct']['unit_id']),'empty'=>[0=>'-- Unidad --'],])."</td>";
