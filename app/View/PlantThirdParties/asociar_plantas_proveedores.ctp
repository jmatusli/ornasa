...
<div class="asociarplantasproveedores fullwidth" style="overflow-x:auto">
<?php 
	echo $this->Form->create('PlantThirdParty');
	echo "<fieldset>";
		echo $this->Form->input('plant_id',['label'=>'Plant','default'=>$selectedPlantId,'empty'=>[0=>'-- Planta --']]);
		echo $this->Form->input('third_party_id',['label'=>'Proveedor','default'=>$selectedThirdPartyId,'empty'=>[0=>'-- Proveedor --']]);
		echo $this->Form->Submit(__('Actualizar'),['id'=>'refresh','name'=>'refresh']);
		echo "<legend>".__('Asociar Proveedores con Plantas')."</legend>";
		echo $this->Form->Submit(__('Guardar'),['id'=>'submit','name'=>'submit']);	
    echo "<br/>";
    echo $this->Html->link(__('Guardar como Excel'),['action' => 'guardarAsociacionesPlantasProveedores'],['class' => 'btn btn-primary']); 
    ...
    $tableHead.="<thead>";
      $tableHead.="<tr>";
        $tableHead.="<th>".__('Proveedores')."</th>";
    ...
    echo $table;
    $excelOutput.="<table id='plantas_proveedores'>".$excelHead.$excelBody."</table>";
  
  $_SESSION['resumenAsociacionesPlantasProveedores'] = $excelOutput;
