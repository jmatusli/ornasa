<script>
  ...
  $('body').on('change','#ProductProductionTypeId',function(){	
    switch(parseInt($(this).val())){
      case <?php echo PRODUCTION_TYPE_PET; ?>:
        ...
        break;
      case <?php echo PRODUCTION_TYPE_INJECTION; ?>:
  ...
  $('body').on('click','#saveRecipe',function(){
    var recipeProductId=<?php echo PLACEHOLDER_ID; ?>;
  ...
  function showRecipeData(recipeId){
    $.ajax({
      ...
      data:{"recipeId":recipeId,"productId":<?php echo PLACEHOLDER_ID; ?>,"referrerForm":'Product'},
      cache: false,
      type: 'POST',
  ...
  echo "<div class='container-fluid'>";
    echo  "<div class='row'>";
      echo "<div class='col-sm-9'>";  
        echo $this->Form->input('product_type_id',['default'=>0,'empty'=>[0=>'-- Seleccione Tipo de Producto --']]);
        echo $this->Form->input('product_nature_id',['default'=>0,'empty'=>[0=>'-- Seleccione Naturaleza --']]);
        echo $this->Form->input('production_type_id',['default'=>0,'empty'=>[0=>'-- Seleccione Tipo de Producción --'],'div'=>['class'=>'productiontype']]);
        
        //echo $this->Form->Submit('Applicar tipo producción',['id'=>'selectProductionType','name'=>'selectProductionType','div'=>['class'=>'productiontype']]);
        
        echo $this->Form->input('name');
        echo $this->Form->input('abbreviation');
        echo $this->Form->input('unit_id',['default'=>1]);
        echo $this->Form->input('description');
        echo $this->Form->input('bool_active',['value'=>true,'type'=>'hidden']);
        echo $this->Form->input('packaging_unit',['default'=>0]);
        echo $this->Form->input('accounting_code_id',['default'=>0,'style'=>'font-size:0.9em','empty'=>[0=>'-- Seleccione cuenta contable --']]);
      echo '</div>';
      echo '<div class="col-sm-3" style="padding-left:10px">';
        ...
        echo "</ul>";
        /*
        echo '<h3 class="productiontype" style="background-color:pink">Tipo de Producción</h3>';
        foreach ($productionTypes as $productionTypeId=>$productionTypeName){
          echo $this->Form->input('ProductionType.'.$productionTypeId.'.bool_assigned',[
            ...
            'checked'=>(count($productionTypes) == 1?true:false),
            'div'=>['class'=>'div input checkboxleftbig productiontype'],
          ]);
        }
        */
      echo "</div>";   
      ...
      echo "<div class='col-sm-6'>";
        ...
        foreach ($warehouses as $warehouseId=>$warehouseName){
          echo $this->Form->input('Warehouse.'.$warehouseId.'.bool_assigned',[
            'label'=>$warehouseName,
            'type'=>'checkbox',
            'checked'=>(count($warehouses) == 1?true:false),
    ...
    echo  '<div class="row"  style="padding-left:20px;background-color:#eeeeee">';
      ...
      echo '<div class="col-sm-6 price pet injection hidden">';
        ...
        foreach ($priceClientCategories as $priceClientCategoryId=>$priceClientCategoryName){
          echo '<div class="row"  style="margin:0">';
            echo '<div class="col-sm-8">';
              echo $this->Form->input('PriceClientCategory.'.$priceClientCategoryId.'.category_price',['label'=>'Precio venta '.$priceClientCategoryName,'type'=>'decimal','default'=>0,'div'=>['class'=>'input number label50']]);  
        ...
        echo $this->Form->input('threshold_volume',['label'=>'Volumen Venta','default'=>100000,'div'=>['class'=>'input number label50']]);  
