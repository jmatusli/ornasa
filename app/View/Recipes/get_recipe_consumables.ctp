<?php
  if (empty($recipeConsumables)){
    // no sales order present, code copied directly from crear venta
		echo '<h4>No se encontraron consumibles para esta receta</h4>';
	}
	else {
		$tableHead='';
    $tableHead.="<thead>";
    ...
    $tableRows='';
    $counter=0;
    if(!empty($requestRecipeConsumables['Products'])){
      foreach ($requestRecipeConsumables['Products'] as $requestItem){
        $quantityForOne=0;
        if (array_key_exists('quantity_for_one',$requestItem)){
          ...
          foreach ($recipeConsumables as $recipeConsumable){
            if ($recipeConsumable['RecipeConsumable']['product_id'] == $requestItem['product_id']){
              $quantityForOne= $recipeConsumable['RecipeConsumable']['quantity'];
            }
          }
        }
        if ($quantityForOne == 0){
        ...
        $tableRow='';
        $tableRow.='<tr class="consumablerow">';
          $tableRow.='<td class="consumableid">';
            $tableRow.=$this->Form->Input('RecipeConsumable.'.$counter.'.product_id',[
              'label'=>false,
          ...
          $tableRow.='</td>';
          $tableRow.='<td class="consumablequantity">';
            $tableRow.=$this->Form->Input('RecipeConsumable.'.$counter.'.quantity_for_one',[
              //'label'=>false,
              'value'=>$quantityForOne,
              'class'=>'unitquantity',
              'type'=>'hidden',
            ]);
            $tableRow.=$this->Form->Input('RecipeConsumable.'.$counter.'.quantity',[
              'label'=>false,
          ...
          $tableRow.='</td>';
          $tableRow.='<td>'.$this->Form->Input('RecipeConsumable.'.$counter.'.unit_id',[
            'label'=>false,
      ...
      foreach ($recipeConsumables as $recipeConsumable){
        ...
        $tableRow.='<tr class="consumablerow">';
          $tableRow.='<td class="consumableid">';
            $tableRow.=$this->Form->Input('RecipeConsumable.'.$counter.'.product_id',[
              'label'=>false,
              'default'=>$recipeConsumable['RecipeConsumable']['product_id'],
              //'type'=>'hidden',
            ]);
            //$tableRow.=$recipeConsumable['Product']['name'];
          $tableRow.='</td>';
          $tableRow.='<td class="consumablequantity">';
            $tableRow.=$this->Form->Input('RecipeConsumable.'.$counter.'.quantity_for_one',[
              'value'=>$recipeConsumable['RecipeConsumable']['quantity'],
              'class'=>'unitquantity',
              'type'=>'hidden',
            ]);
            $tableRow.=$this->Form->Input('RecipeConsumable.'.$counter.'.quantity',[
              'label'=>false,
              'value'=>$finishedProductQuantity*$recipeConsumable['RecipeConsumable']['quantity'],
              'class'=>'quantity',
            ]);
          $tableRow.='</td>';
          $tableRow.='<td>'.$this->Form->Input('RecipeConsumable.'.$counter.'.unit_id',[
            'label'=>false,
            'value'=>$recipeConsumable['RecipeConsumable']['unit_id'],
            'class'=>'fixed',
            'options'=>$units,
          ]).'</td>';
        $tableRow.='</tr>';
    ...
    /*
      ...
      for ($i=0;$i<count($productsForSalesOrder);$i++) { 
        ...
        echo "</tr>";
			echo "</tbody>";
    */  
		echo '<table id ="Consumables">'.$tableHead.$tableBody.'</table>';
	}
