...
<div class="productionTypeProducts index">
<?php 
	echo '<h2>'.__('Production Type Products').'</h2>';
...
<?php
	$tableHeader='<thead>';
		$tableHeader.='<tr>';
			...
			$tableHeader.='<th>'.$this->Paginator->sort('production_type_id').'</th>';
	...
	$excelHeader='<thead>';
		$excelHeader.='<tr>';
			...
			$excelHeader.='<th>'.$this->Paginator->sort('production_type_id').'</th>';
	...
	foreach ($productionTypeProducts as $productionTypeProduct){ 
		$tableRow='';		$tableRow.='<td>'.h($productionTypeProduct['ProductionTypeProduct']['assignment_datetime']).'</td>';
		$tableRow.='<td>'.$this->Html->link($productionTypeProduct['ProductionType']['name'], ['controller' => 'production_types', 'action' => 'detalle', $productionTypeProduct['ProductionType']['id']]).'</td>';
		$tableRow.='<td>'.$this->Html->link($productionTypeProduct['Product']['name'], ['controller' => 'products', 'action' => 'detalle', $productionTypeProduct['Product']['id']]).'</td>';

			$excelBody.='<tr>'.$tableRow.'</tr>';

			$tableRow.='<td class="actions">';
				$tableRow.=$this->Html->link(__('View'), ['action' => 'detalle', $productionTypeProduct['ProductionTypeProduct']['id']]);
		if ($bool_edit_permission){
				$tableRow.=$this->Html->link(__('Edit'), ['action' => 'editar', $productionTypeProduct['ProductionTypeProduct']['id']]);
		}
			$tableRow.='</td>';
