...
<div class="productionTypes index">
<?php 
	echo '<h2>'.__('Production Types').'</h2>';
  ...
  $fileName=($nowDateTime->format('YMD')).'_Resumen_Tipos Producción.xlsx';
	echo $this->Html->link(__('Guardar como Excel'), ['action' => 'guardarResumenTiposProducción',$fileName], ['class' => 'btn btn-primary']);
?> 
</div>
<div class="actions">
<?php 
...
<?php
	...
	foreach ($productionTypes as $productionType){ 
		$tableRow='';		$tableRow.='<td>'.$this->Html->link($productionType['ProductionType']['name'],['action' => 'detalle', $productionType['ProductionType']['id']]).'</td>';
		$tableRow.='<td>'.h($productionType['ProductionType']['short_description']).'</td>';
		//$tableRow.='<td>'.h($productionType['ProductionType']['long_description']).'</td>';
		$tableRow.='<td>'.h($productionType['ProductionType']['list_order']).'</td>';
		$tableRow.='<td'.(empty($productionType['ProductionType']['hex_color'])?'':' style="background-color:#'.$productionType['ProductionType']['hex_color'].';"').'>'.h($productionType['ProductionType']['hex_color']).'</td>';
    $excelBody.='<tr>'.$tableRow.'</tr>';

    $tableRow.='<td class="actions">';				
    if ($bool_edit_permission){
        $tableRow.=$this->Html->link(__('Edit'), ['action' => 'editar', $productionType['ProductionType']['id']]);
    }
    $tableRow.='</td>';
	...
	$_SESSION['resumenTiposProduccion'] = $excelOutput;
?>
</div>