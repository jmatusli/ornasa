    ...
    foreach ($machineUtility['outputTotals'] as $rawMaterialId=>$rawMaterialData){
      ...
      foreach ($rawMaterialData as $productId=>$productData){
        ...
        $productRowDetail.="<tr style='background-color:lightgreen;'>";
            ...
            $productRowDetail.="<td colspan=2>".(array_key_exists($productId,$finishedProductList) ? $finishedProductList[$productId]:(array_key_exists($productId,$rawMaterialList) ? $rawMaterialList[$productId]:"-"))."</td>";
        ...
        $productRowSummary.="<tr>";
          ...
          $productRowSummary.="<td>".(array_key_exists($productId,$finishedProductList) ? $finishedProductList[$productId]:(array_key_exists($productId,$rawMaterialList) ? $rawMaterialList[$productId]:"-"))."</td>";
	...
	if (!empty($producedProductsPerOperator)){
        ...
        //pr($machine);
				$totalRows.="<tr class='totalrow'>";
					$totalRows.="<td>Porcentajes</td>";
					for ($i=0;$i<count($totalsArray);$i++){
            if ($machine['Machine']['plant_id'] == PLANT_SANDINO && $i <3){
              if ($i%3==0){
			...
			echo "<tbody>";
				...
				foreach ($machine['ProductionRun'] as $productionRun){
					...
					$productionRunRows.="<tr>";
						...
						$productionRunRows.="<td>".(empty($productionRun['RawMaterial']['name'])?"-":$productionRun['RawMaterial']['name'])."</td>";
