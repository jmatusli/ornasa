...
<?php 
    ...
    echo "<tbody>";
        ...
        echo '<td>'.$this->Html->link($machine['Plant']['name'],['controller'=>'plants','action'=>'detalle',$machine['Plant']['id']]).'</td>';
				echo "<td>".$this->Html->link($machine['Machine']['name'].($machine['Machine']['bool_active']?"":" (Deshabilitada)"),['action'=>'detalle',$machine['Machine']['id']])."</td>";
				echo "<td>".$machine['Machine']['description']."</td>";
				echo "<td class='actions'>";
					if($bool_edit_permission){
						echo $this->Html->link(__('Edit'), ['action' => 'editar', $machine['Machine']['id']]); 
					}
...
<?php
	...
	echo "<ul>";
		if ($bool_add_permission) {
			echo "<li>".$this->Html->link(__('New Machine'), ['action' => 'crear'])."</li>";
			echo "<br/>";
		}
    ...
    foreach ($machines as $machine){
      ...
      foreach ($machineUtility['outputTotals'] as $rawMaterialId=>$rawMaterialData){
        ...
        foreach ($rawMaterialData as $productId=>$productData){
          ...
          //if (!array_key_exists($productId,$finishedProductList)){
          ...
          $productRowSummary.="<tr>";
            ...
            $productRowSummary.="<td></td>";
            $productRowSummary.="<td>".(array_key_exists($productId,$finishedProductList)?$finishedProductList[$productId]:$otherProductList[$productId])."</td>";
