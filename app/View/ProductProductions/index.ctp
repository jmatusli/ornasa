<div class="productProductions index">
<?php 
	echo "<h2>".__('Product Productions')."</h2>";
...
<?php
	$pageHeader="<thead>";
		$pageHeader.="<tr>";
			$pageHeader.="<th>".$this->Paginator->sort('application_date')."</th>";
			$pageHeader.="<th>".$this->Paginator->sort('product_id')."</th>";
			$pageHeader.="<th>".$this->Paginator->sort('acceptable_production')."</th>";
			$pageHeader.="<th class='actions'>".__('Actions')."</th>";
		$pageHeader.="</tr>";
	$pageHeader.="</thead>";
	$excelHeader="<thead>";
		$excelHeader.="<tr>";
			$excelHeader.="<th>".$this->Paginator->sort('application_date')."</th>";
			$excelHeader.="<th>".$this->Paginator->sort('product_id')."</th>";
			$excelHeader.="<th>".$this->Paginator->sort('acceptable_production')."</th>";
		$excelHeader.="</tr>";
	$excelHeader.="</thead>";
	...
	foreach ($productProductions as $productProduction){ 
		$pageRow=""		$pageRow=""		$pageRow.="<td>".h($productProduction['ProductProduction']['application_date'])."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($productProduction['Product']['name'], array('controller' => 'products', 'action' => 'view', $productProduction['Product']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".h($productProduction['ProductProduction']['acceptable_production'])."</td>";
		$pageRow=""		$pageRow=""
			$excelBody.="<tr>".$pageRow."</tr>";

			$pageRow.="<td class='actions'>";
				$pageRow.=$this->Html->link(__('View'), array('action' => 'view', $productProduction['ProductProduction']['id']));
				$pageRow.=$this->Html->link(__('Edit'), array('action' => 'edit', $productProduction['ProductProduction']['id']));
				//$pageRow.=->postLink(__('Delete'), array('action' => 'delete', $productProduction['ProductProduction']['id']), array(), __('Are you sure you want to delete # %s?', $productProduction['ProductProduction']['id']));
			$pageRow.="</td>";
