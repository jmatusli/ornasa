...
<?php 
	echo "<h1>".__('Reporte de Proveedores por Pagar')."</h1>";
	echo $this->Html->link(__('Guardar como Excel'),['action' => 'guardarProveedoresPorPagar'], ['class' => 'btn btn-primary']); 
  echo '<p class="info">Los días de  crédito se cuentan a partir del día de entrada o el día de la factura, según el caso</p>';
	$reportTable="";
	...
	$reportTable.="<table cellpadding='0' cellspacing='0' id='".$table_id."'>";
		...
		foreach ($providers as $provider){
			...
			if ($provider['saldo']>0){
				$providerBody.="<tr>";
					$providerBody.="<td>".$this->Html->link($provider['ThirdParty']['company_name'], ['controller' => 'orders', 'action' => 'verFacturasPorPagar', $provider['ThirdParty']['id']])."</td>";
