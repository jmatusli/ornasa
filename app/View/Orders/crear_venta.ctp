...
<script>
  ...
  var jsOwnerUsersPerClient=<?php  echo json_encode($ownerUsersPerClient); ?>;
  ...
  function setInvoiceCode(){
    $('#clientProcessMsg').html('Calculando el código de la venta');
		showMessageModal();
  
		var warehouseId=parseInt($('#OrderWarehouseId').val());
		if (warehouseId>0){
			$.ajax({
				...
				success: function (invoiceCode) {
					$('#OrderOrderCode').val(invoiceCode);
          $('#mainform legend').html("Crear Nueva Venta "+invoiceCode);
          hideMessageModal()
				},
				error: function(e){
      ...
      $('#OrderOrderCode').val('');
      $('#mainform legend').html("Crear Nueva Venta");
      hideMessageModal()
    }
	}
    ...
    if (parseInt(salesOrderId)>0){
      ...
      updatePrices=false;
        ...
        dataType:'json',
          ...
          var creditAuthorizationUserId=salesOrder.SalesOrder.credit_authorization_user_id;
      ...
      $('#CreditData').removeClass('hidden');
        ...
        success: function (clientdata) {
          ...
          */
          ...
          var creditAuthorizationUserId=$('#OrderCreditAuthorizationUserId').val();
          if (creditAuthorizationUserId){     
  ...
  $('body').on('click','#EditClientSave',function(){
    ...
    var clientFirstName=$('#EditClientFirstName').val();
    var clientLastName=$('#EditClientLastName').val();
		var clientEmail=$('#EditClientEmail').val();
    var clientPhone=$('#EditClientPhone').val();
    var clientRucNumber=$('#EditClientRucNumber').val();
    var clientTypeId=$('#EditClientClientTypeId').val();
    var clientZoneId=$('#EditClientZoneId').val();
    var clientAddress=$('#EditClientAddress').val();
    $.ajax({
			url: '<?php echo $this->Html->url('/'); ?>thirdParties/saveexistingclient/',
      data:{"clientId":clientId,"clientFirstName":clientFirstName,"clientLastName":clientLastName,"clientEmail":clientEmail,"clientPhone":clientPhone,"clientRucNumber":clientRucNumber,"clientTypeId":clientTypeId,"clientZoneId":clientZoneId,"clientAddress":clientAddress},
			cache: false,
			type: 'POST',
			success: function (data) {
				if (data=="1"){
          ...
          $('#OrderClientEmail').val(clientEmail);
          $('#OrderClientPhone').val(clientPhone);
          $('#OrderClientRuc').val(clientRucNumber);
          $('#OrderClientTypeId').val(clientTypeId);
          $('#OrderZoneId').val(clientZoneId);
          $('#OrderClientAddress').val(clientAddress);
				}
        else {
          console.log(data);
  ...
  function showServiceCosts(){
    ...
    if (<?php echo ($boolInitialLoad?1:0); ?> == 1 && <?php echo $salesOrderId; ?> >0){
      ...
      if ($('#OrderThirdPartyId').val() > 0 &&   $('#OrderClientGeneric').val() != 1 ){ 
        updatePrices=false;
      ...
      showMessageModal();
        ...
        dataType:'json',
				...
				success: function (salesOrder) {
					if (adminUserIds.indexOf(parseInt(salesOrder.SalesOrder.credit_authorization_user_id)) != -1){
            boolCreditAuthorized=1;
            var clientId=salesOrder.SalesOrder.client_id;
            var creditAuthorizationUserId=salesOrder.SalesOrder.credit_authorization_user_id;
            var creditApplied=salesOrder.SalesOrder.bool_credit?1:0;
            if (creditAuthorizationUserId){     
              setClientCreditData(clientId,creditApplied,creditAuthorizationUserId);
  ...
  echo "</div>";
	...
	echo "<fieldset>";
		...
		echo "<div class='container-fluid'>";
          ...
          echo $this->Form->Submit('Actualizar Bodega',['id'=>'refresh','name'=>'refresh']);
          ...
          //}
          //else {
          //  echo $this->Form->input('vendor_user_id',['default'=>$vendorUserId]);
          //}
          ...
          echo $this->Form->input('delivery_id',['type'=>'hidden','default'=>0]);
        ...
        echo "<div class='col-sm-12'>";  
          ...
          echo "<h2>Productos en Venta</h2>";
            ...
            for ($i=0;$i<count($requestProducts);$i++) { 
              echo "<tr row='Fact_".$i."'>";
                echo "<td class='productid'>".$this->Form->input('Product.'.$i.'.product_id',['label'=>false,'value'=>$requestProducts[$i]['Product']['product_id'],'empty' =>[0=>__('Choose a Product')]])."</td>";
                ...
                echo "<td class='productunitprice'>";
                  ...
                  echo $this->Form->input('Product.'.$i.'.default_product_unit_price',[
                    'label'=>false,
                    'type'=>'hidden',
                    'value'=>$requestProducts[$i]['Product']['default_product_unit_price'],
            ...
            for ($i=count($requestProducts);$i<QUOTATION_ARTICLES_MAX;$i++) { 
              if ($i==count($requestProducts)){
                ...
                echo "<td class='productid'>".$this->Form->input('Product.'.$i.'.product_id',['label'=>false,'default'=>'0','empty' =>[0=>__('Choose a Product')]])."</td>";
