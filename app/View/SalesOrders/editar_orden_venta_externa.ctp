...
/*  
  ...
  function setSalesOrderCode(){
    $('#clientProcessMsg').html('Calculando el código de la orden de venta');
		...
		if (warehouseId>0){
			$.ajax({
				worlurl: '<?php echo $this->Html->url('/'); ?>salesOrders/getSalesOrderCode/',
      ...
      updatePrices=false;
        ...
        dataType:'json',
          ...
          if (!boolClientGeneric){
            if (jsClientRucNumbers[quotation.Quotation.client_id]>0){
...
*/	
  ...
  function setClientCreditData(clientId,creditApplied,creditAuthorizationUserId,boolUpdateProductPrices=0){	
  ...
  $('body').on('click','.showPriceSelection',function(){
    ...
    var clientId=$('#SalesOrderClientId').val();
    var creditApplied=<?php echo $this->request->data['SalesOrder']['bool_credit']?1:0; ?>;
    var creditAuthorizationUserId=<?php echo $creditAuthorizationUserId; ?>;
    if (jQuery.inArray(clientId,Object.values(jsGenericClientIds)) != -1){
      $('#SalesOrderClientGeneric').val(1);
    }
    if (clientId > 0){
      updatePrices=false;
      setClientCreditData(clientId,creditApplied,creditAuthorizationUserId);
...
<?php
	...
	echo '<fieldset style="font-size:1.2em;">';
		echo '<legend>Editar Orden de Venta Externa '.$this->request->data['SalesOrder']['sales_order_code'].'</legend>';
		echo "<div class='container-fluid'>";
			echo "<div class='row'>";	
				echo '<div class="col-md-4" style="padding:5px;">';
					echo '<h3>Orden de Venta</h3>';
          ...
          echo $this->Form->input('quotation_id',[
            ...
            'style'=>'background-color:'.(count($quotations)>0?'yellow':'none'),
					...
					echo $this->Form->input('sales_order_code',['readonly'=>'readonly','style'=>'min-width:165px;']);
          ...
          echo $this->Form->input('bool_delivery',[
            ...
            'onclick'=>(empty($this->request->data['SalesOrder']['delivery_id'])?' return true;':'return false;'),
          ]);
          echo $this->Form->input('delivery_id',['type'=>'hidden']);
          if (!empty($this->request->data['SalesOrder']['delivery_id'])){
            echo '<p class="alert">No se puede remover la entrega a domicilio porque ya está programada</p>';
          }
          ...
          echo $this->Form->input('delivery_address',['label'=>'Dirección de entrega', 'type'=>'textarea','div'=>['class'=>($boolDelivery?'input textarea':'input textarea d-none')]]);
        ...
        echo '<div class="col-md-4" style="padding:5px;">';
          ...
          echo '<div class="topright" style="font-size:18px;">';
						echo '<dl>';
							echo '<dt>Subtotal</dt>';
							echo '<dd id="subtotal" style="clear:right;"><span class="currency">C$</span><span class="amountright">'.$this->request->data['SalesOrder']['price_subtotal'].'</span></dd>';
							echo '<dt style="clear:left;">IVA</dt>';
							echo '<dd id="iva" style="clear:right;"><span class="currency">C$</span><span class="amountright">'.$this->request->data['SalesOrder']['price_iva'].'</span></dd>';
              echo '<dt style="clear:left;">Total</dt>';
							echo '<dd id="total" style="clear:right;"><span class="currency">C$</span><span class="amountright">'.$this->request->data['SalesOrder']['price_total'].'</span></dd>';
						echo '</dl>';
            echo '<br/>';
            echo '<dl>';
              echo '<dt style="clear:left;">Retención</dt>';
							echo '<dd id="retention" style="clear:right;"><span class="currency">C$</span><span class="amountright">'.$this->request->data['SalesOrder']['retention_amount'].'</span></dd>';
            echo '</dl>';
            echo '<br/>';
            echo $this->Form->input('currency_id',['default'=>$currencyId,'type'=>'hidden']);
            //echo $this->Form->input('product_type_id',['label'=>'Línea de Producto','default'=>$productTypeId]);
            //echo $this->Form->Submit(__('Cambiar línea de producto'),[
        ...
        echo "<table id='salesOrderProducts' style='font-size:16px;'>";
          ...
          for ($sop=0;$sop<count($requestProducts);$sop++){
            echo "<tr row='".$sop."'>";
              ...
              echo "<td class='productunitprice'>";
                ...
                echo $this->Form->input('SalesOrderProduct.'.$sop.'.default_product_unit_price',[
                  'label'=>false,
                  'type'=>'hidden',
                  'value'=>(empty($requestProducts[$sop]['SalesOrderProduct']['default_product_unit_price'])?$requestProducts[$sop]['SalesOrderProduct']['product_unit_price']:$requestProducts[$sop]['SalesOrderProduct']['default_product_unit_price']),
          ...
          for ($sop=$counter;$sop<QUOTATION_ARTICLES_MAX;$sop++){
            ...
            echo "<tr class='totalrow subtotal'>";
              ...
              echo "<td></td>";
              echo "<td class='totalprice amount right'><span class='currency'></span>".$this->Form->input('price_subtotal',['label'=>false,'type'=>'decimal','readonly'=>'readonly','default'=>$this->request->data['SalesOrder']['price_subtotal']])."</td>";
              echo "<td></td>";
            echo "</tr>";		
            echo "<tr class='iva'>";
              echo "<td>IVA</td>";
              ...
              echo "<td></td>";
              echo "<td></td>";
              echo "<td class='totalprice amount right'><span class='currency'></span>".$this->Form->input('price_iva',['label'=>false,'type'=>'decimal','readonly'=>'readonly','default'=>$this->request->data['SalesOrder']['price_iva']])."</td>";
              echo "<td></td>";
            echo "</tr>";		
            echo "<tr class='totalrow total'>";
              ...
              echo "<td></td>";
              echo "<td></td>";
              echo "<td class='totalprice amount right'><span class='currency'></span>".$this->Form->input('price_total',['label'=>false,'type'=>'decimal','readonly'=>'readonly','default'=>$this->request->data['SalesOrder']['price_total']])."</td>";
              echo "<td></td>";
            echo "</tr>";		
            echo "<tr class='retention'>";
              ...
              echo "<td></td>";
              echo "<td></td>";
              echo "<td class='totalprice amount right'><span class='currency'></span>".$this->Form->input('retention_amount',['label'=>false,'type'=>'decimal','readonly'=>'readonly','default'=>$this->request->data['SalesOrder']['retention_amount']])."</td>";
              echo "<td></td>";
            echo "</tr>";		
          echo "</tbody>";
        echo "</table>";
