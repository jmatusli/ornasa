...
<script>
  ...
  var jsProductTypesPerProduct=<?php echo json_encode($productTypesPerProduct); ?>;
	...
	$('body').on('change','#QuotationVendorUserId',function(){	
		getQuotationCode();
	});
    ...
    $('#clientProcessMsg').html('Calculando el código de la cotización');
		showMessageModal();
  
		var userId=$('#QuotationVendorUserId').val();
    ...
    var warehouseId=parseInt($('#QuotationWarehouseId').val());
		if (warehouseId>0){
      $.ajax({
        url: '<?php echo $this->Html->url('/'); ?>quotations/getQuotationCode/',
        data:{"userId":userId,"quotationDateDay":quotationDateDay,"quotationDateMonth":quotationDateMonth,"quotationDateYear":quotationDateYear,"warehouseId":warehouseId},
        cache: false,
        type: 'POST',
        success: function (quotationCode) {
          $('#QuotationQuotationCode').val(quotationCode);
          $('#QuotationCrearForm legend').html("Crear Nueva Cotización "+quotationCode);
          hideMessageModal()
        },
        error: function(e){
          console.log(e);
      ...
      $('#QuotationQuotationCode').val('');
      $('#QuotationCrearForm legend').html("Crear Nueva Cotización");
      hideMessageModal()
    }
	}
  ...
  $('body').on('change','#SetSaveAllowed',function(){	
    ...
    if (allowSaving){
      ...
      $('#CreditUsername').val("Crédito de cliente")
    }
  });
  ...
  function setClientCreditData(clientId,creditApplied){	
    ...
    creditAuthorizationUserId=$("#CreditAuthorizationUserId").val()
  ...
  $('body').on('change','#QuotationCurrencyId',function(){	
    ...
    getQuotationCode();
		...
		if ($('#QuotationClientId').val() > 0 && $('#QuotationClientGeneric').val() != 1){    
      updatePrices=false;
  ...
  $('body').on('submit','#QuotationCrearForm',function(e){	
    if($(".save").data('clicked')){
      $('.save').attr('disabled', 'disabled');
      $("#QuotationCrearForm").fadeOut();
...
<?php 
  echo "<div id='saving' style='min-height:180px;z-index:9998!important;position:relative;'>";
    echo "<div id='savingcontent'  style='z-index:9999;position:relative;'>";
      echo "<p id='savingspinner' style='font-weight:700;font-size:24px;text-align:center;z-index:100!important;position:relative;'>Guardando la cotización...</p>";
    echo "</div>";
  echo "</div>";
	...
	echo '<fieldset style="font-size:1.2em;">';
		...
		echo "<div class='container-fluid'>";
          ...
          if ($userRoleId == ROLE_ADMIN){
            echo $this->Form->input('vendor_user_id',['default'=>$loggedUserId,'options'=>$users]);
          }
          else {
            echo $this->Form->input('vendor_user_id',['default'=>$loggedUserId,'type'=>'hidden','options'=>$users]);
          }	
          ...
          echo $this->Form->input('Quotation.client_phone',['label'=>'Teléfono','type'=>'phone']);
          echo $this->Form->input('Quotation.client_email',['label'=>'Correo','type'=>'email']);
          //echo $this->Form->input('client_ruc',['label'=>'RUC']);
          echo $this->Form->input('Quotation.client_type_id',['default'=>0,'empty'=>[0=>'-- Tipo de Cliente --']]);
          echo $this->Form->input('Quotation.zone_id',['default'=>0,'empty'=>[0=>'-- Zona --']]);
          ...
          echo '<div class="topright" style="font-size:18px;">';
            ...
            echo $this->Form->input('currency_id',['default'=>$currencyId,'type'=>'hidden']);
					...
					echo "</div>";
          //echo '<h3>Remarca</h3>';
          //echo $this->Form->input('QuotationRemark.user_id',['label'=>'Vendedor','value'=>$loggedUserId,'type'=>'hidden']);
					//echo $this->Form->input('QuotationRemark.remark_text',['rows'=>2,'required'=>'required','default'=>'Cotización creada']);
					//echo $this->Form->input('QuotationRemark.working_days_before_reminder',['default'=>5]);
					//echo $this->Form->input('QuotationRemark.reminder_date',['dateFormat'=>'DMY','minYear'=>2014, 'maxYear'=>date('Y')+1]);
					//echo $this->Form->input('QuotationRemark.action_type_id',['default'=>ACTION_TYPE_OTHER]);
				echo "</div>";
			echo "</div>";
		echo "</div>";
    ...
    echo "<div>";
      ...
      echo "<div id='productsContainer'>";
        echo "<table id='quotationProducts' style='font-size:16px;'>"; 
          ...
          if (count($requestProducts)>0){
            for ($i=0;$i<count($requestProducts);$i++) { 
              ...
              echo "<tr row='".$i."'>";
                ...
                echo "<td class='rawmaterialid'>".$this->Form->input('QuotationProduct.'.$i.'.raw_material_id',['label'=>false,'value'=>$requestProducts[$i]['QuotationProduct']['raw_material_id'],'empty' =>[0=>__('-- Preforma --')]])."</td>";
                ...
                echo "<td class='productunitprice amount'>";
                  ...
                  echo $this->Form->input('QuotationProduct.'.$i.'.default_product_unit_price',['label'=>false,'type'=>'hidden','value'=>$requestProducts[$i]['QuotationProduct']['default_product_unit_price'],'class'=>'defaultproductprice']);
          ...
          for ($j=$counter;$j<QUOTATION_ARTICLES_MAX;$j++) { 
            if ($j==$counter){
              ...
              echo "<td class='rawmaterialid'>".$this->Form->input('QuotationProduct.'.$j.'.raw_material_id',array('label'=>false,'default'=>'0','empty' =>[0=>__('-- Preforma --')]))."</td>";
              //echo "<td class='productimage'></td>";
		...
		//echo $this->Form->input('bool_print_delivery_time',['label'=>'Mostrar tiempo de entrega en pdf?','checked'=>false]);
		
		//echo $this->Form->input('payment_form',['label'=>'Forma de Pago','default'=>'50 % al ordenar y 50 % contra entrega']);
		//echo $this->Form->input('remark_delivery',['label'=>'Remarca sobre entrega','default'=>'Los días de entrega del producto es a partir del arte aprobado y con orden de compra']);
		echo $this->Form->input('remark_cheque',['label'=>'Remarca sobre cheque','default'=>'El cheque será emitido a nombre de Orna s.a.']);
		//echo $this->Form->input('remark_elaboration',['label'=>'Remarca sobre elaboración','default'=>'Se procederá a la elaboración de su producto una vez recibida la autorización del arte y cotización']);
		echo $this->Form->input('text_client_signature',['label'=>'Etiqueta firma cliente','default'=>'Firma Autorizada del Cliente']);
		echo $this->Form->input('text_authorization',['label'=>'Etiqueta autorización','default'=>'Nombre y cargo de quien autoriza']);
		echo $this->Form->input('text_seal',['label'=>'Etiqueta sello','default'=>'Sello de la institución']);
		...
		echo '<div class="modal-dialog">';
          ...
          echo '<p id="clientProcessMsg">Calculando datos</p>';
				...
				echo '<div id="modalFooter" class="modal-footer hidden">';
					echo '<button id="closeMsg" type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>';
				echo '</div>';
			echo '</div>';
		echo '</div>';
	echo '</div>';
