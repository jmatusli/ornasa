...
*/
  ...
  function calculateDepositTotal(){
    ...
    $('#DebitAmount').val(depositTotal);
    $('#CreditAmount').val(depositTotal);
  }  
  ...
  $('body').on('keypress','#content',function(){
  ...
  echo "<div class='container-fluid'>";
    echo "<div class='row'>";
      echo "<div class='col-md-7'>";
        ...
        echo $this->Form->input('transfer_code',['class'=>'narrow','default'=>$newTransferCode,'readonly'=>'readonly']);
    ...
    echo "</div>";
    echo "<br/>";
    echo "<p class='comment'>El monto pagado de una factura de contado o recibo de caja se calcula tomando el precio total incluyendo IVA, restándole la retención si es presente</p>";
    ...
    echo $this->Form->input('powerselector1',['class'=>'powerselector','checked'=>false,'style'=>'width:5em;','label'=>['text'=>'Seleccionar/Deseleccionar facturas para usuario','style'=>'padding-left:5em;'],'format' => ['before', 'input', 'between', 'label', 'after', 'error' ]]);
    
    $totalCS=0;
    $totalUSD=0;
    $totalDeposited=0;
    foreach ($undepositedInvoices as $invoice){
      //pr($invoice);
      $invoiceDateTime=new DateTime($invoice['Invoice']['invoice_date']);
      $paymentTableRow="";
      $paymentTableRow.="<tr>";
        $paymentTableRow.="<td class='selector'>".$this->Form->input('Payment.'.$invoice['Invoice']['id'].'.selector',['checked'=>false,'label'=>false])."</td>";
        $paymentTableRow.="<td>".$invoiceDateTime->format('d-m-Y')."</td>";
        $paymentTableRow.="<td class='payment'>".$this->Html->link($invoice['Invoice']['invoice_code'],['controller'=>'invoices','action'=>'view',$invoice['Invoice']['id']],['class'=>'Invoice'])."</td>";
        if ($invoice['Invoice']['currency_id']==CURRENCY_CS){
          $paymentTableRow.="<td class='CScurrency'><span class='currency'></span><span class='amountright'>".($invoice['Invoice']['paid_amount_CS'])."</span></td>";
          $paymentTableRow.="<td class='USDcurrency'><span class='currency'></span><span class='amountright'>0</span></td>";
          $totalCS+=($invoice['Invoice']['paid_amount_CS']);
        }
        elseif ($invoice['Invoice']['currency_id']==CURRENCY_USD){
          $paymentTableRow.="<td class='CScurrency'><span class='currency'></span><span class='amountright'>0</span></td>";
          $paymentTableRow.="<td class='USDcurrency'><span class='currency'></span><span class='amountright'>".($invoice['Invoice']['paid_amount_USD'])."</span></td>";
          $totalUSD+=($invoice['Invoice']['paid_amount_USD']);
        }
        else {
          $paymentTableRow.="<td class='CScurrency'><span class='currency'></span><span class='amountright'>0</span></td>";
          $paymentTableRow.="<td class='USDcurrency'><span class='currency'></span><span class='amountright'>0</span></td>";
        }
        $paymentTableRow.="<td class='paidAmountCs'>".($invoice['Invoice']['paid_amount_CS'])."</td>";
        $paymentTableRow.="<td class='paidAmountUsd'>".($invoice['Invoice']['paid_amount_USD'])."</td>";
        $paymentTableRow.="<td class='deposit_column'><span class='currency'></span><span class='amountright'>";
        $paymentTableRow.=$this->Form->input('Payment.'.$invoice['Invoice']['id'],['type'=>'number','label'=>false,'style'=>'margin-left:10px;']);
    ...
    echo $this->Form->input('powerselector2',['class'=>'powerselector','checked'=>false,'style'=>'width:5em;','label'=>['text'=>'Seleccionar/Deseleccionar facturas para usuario','style'=>'padding-left:5em;'],'format' => ['before', 'input', 'between', 'label', 'after', 'error' ]]);
		...
		echo "<table id='accountingMovementsForRegister'>";
          ...
          echo "<td class='accountingcodeid'>".$this->Form->input('AccountingMovement.'.$i.'.accounting_code_id',['label'=>false,'default'=>ACCOUNTING_CODE_CASHBOX_MAIN,'empty' =>[0=>__('Choose Accounting Code')]])."</td>";
					echo "<td class='concept'>".$this->Form->input('AccountingMovement.'.$i.'.concept',['label'=>false,'value'=>'Depósito'])."</td>";
					echo "<td></td>";
