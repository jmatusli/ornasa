...
*/
    ...
    //pr($purchases);
		...
		$unfinishedProductTypes=$this->ProductType->find('all',[
			'conditions'=>['ProductType.product_category_id'=>[CATEGORY_RAW,CATEGORY_OTHER],],
			'contain'=>[
				'Product'=>[
					'StockMovement'=>[
  ...
  public function resumenEntradasSuministros($lastMonth=0) {
		$this->loadModel('Warehouse');
    $this->loadModel('UserWarehouse');
    ...
    $this->set(compact('warehouseId'));
		...
		$purchaseConditions=[
			'Order.stock_movement_type_id'=> MOVEMENT_PURCHASE_CONSUMIBLES,
			'Order.order_date >='=> $startDate,
			'Order.order_date <'=> $endDatePlusOne,
      'Order.warehouse_id'=>$warehouseId,
		];
    ...
    //pr($purchases);
		...
		$this->loadModel('ProductType');
		$consumibleProductTypes=$this->ProductType->find('all',[
		...
		for ($pt=0;$pt<count($consumibleProductTypes);$pt++){
			for ($p=0;$p<count($consumibleProductTypes[$pt]['Product']);$p++){
				$packagingUnitProduct=$consumibleProductTypes[$pt]['Product'][$p]['packaging_unit'];
				$totalQuantityProduct=0;
				$totalCostProduct=0;
				foreach ($consumibleProductTypes[$pt]['Product'][$p]['StockMovement'] as $stockMovement){
					$totalQuantityProduct+=$stockMovement['product_quantity'];
					$totalCostProduct+=$stockMovement['product_total_price'];
				}
				if ($packagingUnitProduct>0){
					$consumibleProductTypes[$pt]['Product'][$p]['total_packages']=round($totalQuantityProduct/$packagingUnitProduct);
				}
				else {
					$consumibleProductTypes[$pt]['Product'][$p]['total_packages']=-1;
				}
				
				$consumibleProductTypes[$pt]['Product'][$p]['total_quantity_product']=$totalQuantityProduct;
				$consumibleProductTypes[$pt]['Product'][$p]['total_cost_product']=$totalCostProduct;
			}
		}
		//pr($consumibleProductTypes);
		$this->set(compact('consumibleProductTypes'));
      ...
      if (!empty($quantityOthers)){
        foreach ($quantityOthers as $productTypeId=>$productTypeQuantity){
          $summedQuantityOthers+=$productTypeQuantity;
			...
			if ((($quantityProduced+$quantityCap+$quantityService+$quantityImport+$quantityLocal+$summedQuantityOthers)>0)||(!empty($sale['Invoice'])&&($sale['Invoice'][0]['bool_annulled']||empty($sale['StockMovement'])))||(empty($sale['Invoice'])&&empty($sale['CashReceipt'])&&empty($sale['StockMovement']))){
    ...
    $users=$this->User->getActiveVendorUserList();
  ...
  public function resumenComprasRealizadas($clientId=0) {
	...
	public function verEntradaSuministros($id = null) {
  ...
  public function verPdfEntradaSuministros($id = null) {
		if (!$this->Order->exists($id)) {
			throw new NotFoundException(__('Invalid purchase'));
		}
	...
	public function verVenta($id = null) {
		...
		$options = [
			...
			'contain'=>[
				'ThirdParty'=>[
          'fields'=>['ThirdParty.id','ThirdParty.company_name','ThirdParty.bool_generic'],
          'ClientType',
          'Zone',
        ],
        'User',
  ...
  public function imprimirVenta($id = null) {
		...
		$options = [
			...
			'contain'=>[
				'ThirdParty'=>[
          'fields'=>['ThirdParty.id, ThirdParty.company_name, ThirdParty.phone','ThirdParty.address','ThirdParty.ruc_number','ThirdParty.expiration_rate','ThirdParty.first_name','ThirdParty.last_name'],
        ],
        'User'=>[
          ...
          try {
            ...
            foreach ($this->request->data['Product'] as $product){
              ...
              if ($productQuantity>0 && $productId>0){
                ...
                $itemmovementname=$purchaseDate['day']."_".$purchaseDate['month']."_".$purchaseDate['year']."_".$providerName."_".$orderCode."_".$product_name;
                $description="New stockitem ".$product_name." (Quantity:".$productQuantity.",Unit Price:".$productUnitPrice.") from Purchase ".$providerName."_".$orderCode;
    ...
    $plantId=$this->Warehouse->getPlantId($warehouseId);
		...
		$productsAll = $this->Product->find('all',[
			...
			'conditions' => [
				'ProductType.product_category_id !='=> [CATEGORY_PRODUCED,CATEGORY_CONSUMIBLE],
    ...
    $purchaseOrders=$this->PurchaseOrder->getConfirmedPurchaseOrders($warehouseId,0);
  ...
  public function crearEntradaSuministros() {
		$this->loadModel('Product');
		$this->loadModel('StockMovement');
		$this->loadModel('ThirdParty');
		$this->loadModel('ClosingDate');
    ...
    $requestProducts=[];
      ...
      if(empty($this->request->data['refresh'] )){
        ...
        elseif (abs($productTotalSumBasedOnProductTotals-$this->request->data['Order']['total_price']) > 1){
          $this->Session->setFlash('Si se suman los totales de cada producto se llega a '.$productTotalSumBasedOnProductTotals.' pero el total calculado es '.$this->request->data['Order']['total_price'].'.  Verifique que ha indicado cada producto para que se registró un precio y vuelve a intentar.  Si esto no tiene efecto, comuníquese con su administrador.', 'default',['class' => 'error-message']);
        }
        elseif (abs($this->request->data['Order']['subtotal_based_on_products'] - $this->request->data['Order']['total_price']) > 1){
          $this->Session->setFlash('El subtotal basado en productos es '.$this->request->data['Order']['subtotal_based_on_products'].' pero el subtotal de las facturas es '.$this->request->data['Order']['total_price'].'.  No se guardó la entrada.', 'default',['class' => 'error-message']);
        ...
        //}
          ...
          try {
            $this->Order->create();
            $this->request->data['Order']['stock_movement_type_id']=MOVEMENT_PURCHASE_CONSUMIBLES;
            if (!$this->Order->save($this->request->data)) {
              echo "problema guardando la entrada de suministros";
              pr($this->validateErrors($this->Order));
              throw new Exception();
            }
            ...
            $this->recordUserAction($this->Order->id,"add",null);
            // SAVE THE USERLOG FOR THE PURCHASE
            $this->recordUserActivity($this->Session->read('User.username'),"Se registró la entrada de suministros número ".$this->request->data['Order']['order_code']);
            $this->Session->setFlash('Se guardó la entrada de suministros número '.$this->request->data['Order']['order_code'].'.','default',['class' => 'success']);
            return $this->redirect(['action' => 'resumenEntradasSuministros']);
          } 
          catch(Exception $e){
            $datasource->rollback();
            pr($e);
            $this->Session->setFlash('No se guardó la entrada de suministros', 'default',['class' => 'error-message']);
          }
        }
      }
    }
    ...
    $purchaseOrders=$this->PurchaseOrder->getConfirmedPurchaseOrders($warehouseId,1);
		$this->set(compact('purchaseOrders'));
    ...
    if ($this->request->is('post') && empty($this->request->data['refresh'])){	
      ...
      $clientMail=$this->request->data['Order']['client_mail'];
      ...
      if (!empty($this->request->data['Product'])){
        foreach ($this->request->data['Product'] as $product){
          if (!empty($product['product_id']) && $product['product_quantity'] > 0){
            ...
            if ($this->Product->getProductCategoryId($product['product_id']) == CATEGORY_PRODUCED){
      ...
      elseif ($this->request->data['Order']['price_total']==0){
        ...
        if ($salesOrderId > 0){
          ...
          if (!empty($salesOrder)){
            //pr($salesOrder);
            if (in_array($salesOrder['SalesOrder']['user_id'],$adminUserIds) && $salesOrder['SalesOrder']['bool_credit']){
  ...
  */  
            ...
            $_SESSION['remissionRequestData']=$this->request->data;
						...
						try {
							...
							foreach ($products as $product){
								...
								$production_result_code_id =0;
								$raw_material_id=0;
								//echo "product_category_id is ".$product_category_id."<br/>";
								if ($product_category_id==CATEGORY_PRODUCED){
									$production_result_code_id = $product['production_result_code_id'];
									$raw_material_id = $product['raw_material_id'];
								}
								...
								if ($product_category_id==CATEGORY_PRODUCED){
									$usedMaterials= $this->StockItem->getFinishedMaterialsForSale($productId,$production_result_code_id,$product_quantity,$raw_material_id,$remissionDateAsString,$warehouseId);		
								}
								else {
									$usedMaterials= $this->StockItem->getOtherMaterialsForSale($productId,$product_quantity,$remissionDateAsString,$warehouseId);		
								}
								//pr($usedMaterials);
								...
								for ($k=0;$k<count($usedMaterials);$k++){
									...
									// STEP 2: SAVE THE STOCK MOVEMENT
									$message="Se remitió ".$productName." (Cantidad:".$quantity_used.", total para remisión:".$product_quantity.") para Remisión ".$orderCode;
									$StockMovementData=[];
    ...
    */
    ...
    $requestProducts=[];
      ...
      if (empty($this->request->data['refresh'])){
        ...
        elseif (empty($this->request->data['Order']['purchase_order_id'])){
          $this->Session->setFlash(__('Se debe especificar una orden de compra para la entrada!  No se guardó la entrada.'), 'default',['class' => 'error-message']);
        }
        elseif ($boolEditable){
          //pr($this->request->data);
          ...
          try {
            ...
            if (!empty($previousStockMovements)){
              foreach ($previousStockMovements as $previousStockMovement){
                $stockMovementArray=[];
                $stockMovementArray['StockMovement']['id']=$previousStockMovement['StockMovement']['id'];
                $stockMovementArray['StockMovement']['product_quantity']=0;
                $this->Order->StockMovement->id=$previousStockMovement['StockMovement']['id'];
                if (!$this->Order->StockMovement->save($stockMovementArray)) {
                  echo "problema al poner la cantidad de movimiento viejo de entrada en 0";
                  pr($this->validateErrors($this->Order->StockMovement));
                  throw new Exception();
                }
                $stockItemArray=[];
                $stockItemArray['StockItem']['id']=$previousStockMovement['StockMovement']['stockitem_id'];
                $stockItemArray['StockItem']['original_quantity']=0;
                $stockItemArray['StockItem']['remaining_quantity']=0;
                $this->StockItem->id=$previousStockMovement['StockMovement']['stockitem_id'];
                if (!$this->StockItem->save($stockItemArray)) {
                  echo "problema al poner la cantidad de lote viejo de entrada en 0";
                  pr($this->validateErrors($this->StockItem));
                  throw new Exception();
                }
              }
            ...
            $previousOrder=$this->Order->find('first',[
              'fields'=>['Order.purchase_order_id'],
              'conditions'=>['Order.id'=>$id],
              'recursive'=>-1,
            ]);
            ...
            if ($previousOrder['Order']['purchase_order_id'] != $this->request->data['Order']['purchase_order_id']){
              // first check if there are other entries that use this purchaseOrder
              $entriesBesidesThisEntryForThisPurchaseOrder=$this->Order->find('list',[
                'conditions'=>[
                  'Order.id !='=>$id,
                  'Order.purchase_order_id'=>$previousOrder['Order']['purchase_order_id'],
                ]
              ]);
              // restablishing the state of the previous order
              $this->PurchaseOrder->id=$this->request->data['Order']['purchase_order_id'];
              $purchaseOrderArray=[
                'PurchaseOrder'=>[
                  'id'=>$previousOrder['Order']['purchase_order_id'],
                  'purchase_order_state_id'=>(count($entriesBesidesThisEntryForThisPurchaseOrder) == 0?PURCHASE_ORDER_STATE_CONFIRMED_WITH_CLIENT:PURCHASE_ORDER_STATE_RECEIVED_PARTIALLY),
                ],
              ];
              if (!$this->PurchaseOrder->save($purchaseOrderArray)) {
                echo "problema cambiando el estado de la orden de compra";
            ...
            foreach ($this->request->data['Product'] as $product){
              ...
              if ($productQuantity > 0 && $productId > 0){
                ...
                $stockItemData['remaining_quantity']=$productQuantity;
                ...
                $stockMovementData['product_unit_price']=$productUnitPrice;
                $stockMovementData['product_total_price']=$productPrice;
                ...
                $stockItemLogData=[];
                $stockItemLogData['stockitem_id']=$stockItemId;
                $stockItemLogData['stock_movement_id']=$stockMovementId;
                $stockItemLogData['stockitem_date']=$purchaseDate;
                $stockItemLogData['product_id']=$productId;
                $stockItemLogData['product_unit_price']=$productUnitPrice;
                $stockItemLogData['product_quantity']=$productQuantity;
            ...
            if (!empty($previousStockMovements)){
              foreach ($previousStockMovements as $previousStockMovement){
                $this->recreateStockItemLogs($previousStockMovement['StockMovement']['stockitem_id']);
              }
            }
          ...
          //echo "entrada no editable directamente";
          $goodToGo=true;
          // first check if there are products that have been removed and that have been used(present in previous version but not anymore)
          foreach ($productsPurchasedEarlier as $productPurchasedEarlier){
            ...
            if (!$productPresent){
              ...
              $stockItem=$this->StockItem->find('first',['conditions'=>['StockItem.id'=>$productPurchasedEarlier['StockMovement']['stockitem_id']]]);
              if ($stockItem['StockItem']['remaining_quantity']!=$stockItem['StockItem']['original_quantity']){
                ...
                $goodToGo=false;
                $linkedProduct=$this->Product->getProductById($productPurchasedEarlier['StockMovement']['product_id']);
                $productName=$linkedProduct['Product']['name'];
                $warningIfProblem.="Antes era presente el producto ".$productName.", y este producto ya ha estado ocupado en una orden de producción o una venta.";
                $this->Session->setFlash($warningIfProblem, 'default',['class' => 'error-message']);
              }				
            }
          }
          ...
          if ($goodToGo){
            ...
            foreach ($this->request->data['Product'] as $product){
              ...
              if ($productId>0){
                ...
                foreach ($productsPurchasedEarlier as $productPurchasedEarlier){
                  if ($productId==$productPurchasedEarlier['StockMovement']['product_id']){
                    ...
                    $productDifference = $productPurchasedEarlier['StockMovement']['product_quantity']-$productQuantity;
                    ...
                    if ($productDifference > $purchasedRemaining){
                      ...
                      $linkedProduct=$this->Product->getProductById($productId);
                      $productName=$linkedProduct['Product']['name'];
                      $this->Session->setFlash(__('Los cambios no se podían guardar.  Para el producto '.$productName.' ya se han ocupado más productos (en un orden de producción o una salida) que los que se restaron.'),'default',['class' => 'error-message']);
                    }
                    ...
                    if (abs(round($product['product_price']/$product['product_quantity'],2)-$stockItem['StockItem']['product_unit_price'])>=0.005){
                      ...
                      $relatedProductionMovements=$this->ProductionMovement->find('all',[
                        'conditions'=>[
                      ...
                      $relatedStockMovements=$this->StockMovement->find('all',[
                        'conditions'=>[
                      ...
                      if (!empty($relatedProductionMovements)||!empty($relatedStockMovements)){
                        ...
                        $linkedProduct=$this->Product->getProductById($productId);
                        $productName=$linkedProduct['Product']['name'];
                        $this->Session->setFlash(__('Los cambios no se podían guardar.  Para el producto '.$productName.' ya existen ordenes de producción o salidas, y por tanto se prohibe cambiar el costo.'),'default',['class' => 'error-message']);
                      }
                    }
                  }
                }
          ...
          if ($goodToGo){
            ...
            foreach ($productsPurchasedEarlier as $purchaseStockMovement){
              $stockMovementsForStockItemsInThisPurchase=$this->StockMovement->find('all',[
                'conditions'=>[
              ...
              if (count($stockMovementsForStockItemsInThisPurchase)>0){
                ...
                foreach ($stockMovementsForStockItemsInThisPurchase as $stockmovement){
                  ...
                  $movementDateTime=new DateTime($stockmovement['StockMovement']['movement_date']);
                  $warning.="Tapon salió en salida ".$linkedOrder['Order']['order_code']." en ".$movementDateTime->format('d-m-Y')."<br/>";
                }
              }
            }
            if (!$goodToGo){
          ...
          if ($goodToGo){
            ...
            foreach ($productsPurchasedEarlier as $purchaseStockMovement){
              $productionMovementsForStockItemsInThisPurchase=$this->ProductionMovement->find('all',[
                'conditions'=>[
              ...
              if (count($productionMovementsForStockItemsInThisPurchase)>0){
                ...
                foreach ($productionMovementsForStockItemsInThisPurchase as $productionmovement){
                  ...
                  $linkedProductionRun=$this->ProductionRun->find('first',[
                    'conditions'=>[
                      'ProductionRun.id'=>$productionmovement['ProductionMovement']['production_run_id'],
                    ],
                  ]);
                  $movementDateTime=new DateTime($productionmovement['ProductionMovement']['movement_date']);
                  $warning.="Preforma ocupado en orden de producción ".$linkedProductionRun['ProductionRun']['production_run_code']." en ".$movementDateTime->format('d-m-Y')."<br/>";
                }
              }
            }
            if (!$goodToGo){
            ...
            try {
              ...
              foreach ($productsPurchasedEarlier as $productPurchasedEarlier){
                ...
                if (!$productPresent){
                  ...
                  $stockMovement=$this->StockMovement->find('first',['conditions'=>['StockMovement.id'=>$productPurchasedEarlier['StockMovement']['id']]]);
                  ...
                  $stockMovementData['description']=$stockMovement['StockMovement']['description']."|Product removed from order ".$orderCode;
                  ...
                  $stockItemData['description']=$stockItem['StockItem']['id']."|"."Product removed from order ".$orderCode;
                  $stockItemData['stockitem_creation_date']=$purchaseDate;
                  ...
                  $lastStockItemLog =$this->StockItemLog->find('first',[
                    'conditions'=>['StockItemLog.stockitem_id'=>$stockItem['StockItem']['id']],
                  ...
                  // STEP 4: SAVE THE USERLOG FOR THE STOCK MOVEMENT
                  $this->recordUserActivity($this->Session->read('User.username'),'removed product '.$productName.' from purchase order '.$orderCode);
                }
              }
              ...
              foreach ($this->request->data['Product'] as $product){
                ...
                if ($productId>0){
                    ...
                    if ($productId==$productPurchasedEarlier['StockMovement']['product_id']){
                      if (!$boolCorrectionDone){
                        ...
                        if ($productDifference <= $purchasedRemaining){
                          ...
                          $itemMovementName=$purchaseDate['day']."_".$purchaseDate['month']."_".$purchaseDate['year']."_".$providerName."_".$orderCode."_".$productPurchasedEarlier['Product']['name'];
                          $description="Edited stockitem ".$stockItem['StockItem']['name']." for Product ".$productPurchasedEarlier['Product']['name']."(New Quantity:".$productQuantity.",Unit Price:".$productUnitPrice.") from Purchase ".$orderCode;
                          ...
                          $stockItemData['original_quantity']=$productQuantity;
                          $stockItemData['remaining_quantity']=$stockItem['StockItem']['remaining_quantity']-$stockItem['StockItem']['original_quantity']+$productQuantity;
                          ...
                          if (!empty($productionMovementsForThisStockItem)){
                            foreach ($productionMovementsForThisStockItem as $inputProductionMovement){
                              ...
                              $ProductionMovementData['product_unit_price']=$productUnitPrice;
                  ...
                  if (!$purchasedEarlier){
                    ...
                    $message="New stockitem ".$productName." (Quantity:".$productQuantity.",Unit Price:".$productUnitPrice.") from Purchase ".$orderCode;
                    
                    // STEP 1: SAVE THE STOCK ITEM
                    $this->loadModel('StockItem');
                    $stockItemData=[];
                    $stockItemData['name']=$purchaseDate['day']."_".$purchaseDate['month']."_".$purchaseDate['year']."_".$orderCode."_".$productName;
                    $stockItemData['stockitem_creation_date']=$purchaseDate;
                    $stockItemData['description']=$message;
                    $stockItemData['product_id']=$productId;
                    $stockItemData['product_unit_price']=$productUnitPrice;
                    ...
                    $stockMovementData['movement_date']=$purchaseDate;
                    $stockMovementData['bool_input']=true;
                    $stockMovementData['name']=$purchaseDate['day']."_".$purchaseDate['month']."_".$purchaseDate['year']."_".$orderCode."_".$productName;
                    $stockMovementData['description']=$message;
                    $stockMovementData['order_id']=$purchaseId;
                    $stockMovementData['stockitem_id']=$stockItemId;
                    $stockMovementData['product_id']=$productId;
              ...
              $datasource->commit();
              $this->recordUserAction();
              $this->recordUserActivity($this->Session->read('User.username'),"Se editó la entrada # ".$this->request->data['Order']['order_code']);
              $this->Session->setFlash(__('Se editó la entrada.'),'default',['class' => 'success']);
    ...
    //pr($this->request->data['Order']);  
      ...
      'order'=>'Product.name'
		...
		//pr($this->request->data['Order']);
    $purchaseOrders=$this->PurchaseOrder->getConfirmedPurchaseOrders($warehouseId,0,$this->request->data['Order']['purchase_order_id']);
		$this->set(compact('purchaseOrders'));
  ...
  public function editarEntradaSuministros($id = null) {
		if (!$this->Order->exists($id)) {
			throw new NotFoundException(__('Entrada no válida'));
		}
    ...
    $this->set(compact('purchaseOrderDeliveryOptions'));
      ...
      if (empty($this->request->data['refresh'])){      
        ...
        elseif (empty($this->request->data['Order']['purchase_order_id'])){
          ...
          if (abs($productTotalSumBasedOnProductTotals-$this->request->data['Order']['total_price']) > 1){
            $this->Session->setFlash('Si se suman los totales de cada producto se llega a '.$productTotalSumBasedOnProductTotals.' pero el total calculado es '.$this->request->data['Order']['total_price'].'.  Verifique que ha indicado cada producto para que se registró un precio y vuelve a intentar.  Si esto no tiene efecto, comuníquese con su administrador.', 'default',['class' => 'error-message']);
          }
          elseif ($boolEditable){
            $datasource=$this->Order->getDataSource();
            ...
            try {
              ...
              //$linkedProvider=$this->ThirdParty->read(null,$providerId);
              $this->ThirdParty->recursive=-1;
              $linkedProvider=$this->ThirdParty->find('first',array(
                'conditions'=>array(
                  'ThirdParty.id'=>$providerId,
                )
              ));
              $providerName=$linkedProvider['ThirdParty']['company_name'];
              ...
              $datasource->commit();
              $this->recordUserAction();
              $this->recordUserActivity($this->Session->read('User.username'),"Se editó entrada de suministros número ".$this->request->data['Order']['order_code']);
              ...
              $this->Session->setFlash(__('Se editó la entrada de suministros número ').$this->request->data['Order']['order_code'],'default',['class' => 'success']);
              return $this->redirect(['action' => 'resumenEntradasSuministros']);
            }
            catch(Exception $e){
              $this->Session->setFlash(__('La entrada de suministros no se podía editar.'),'default',['class' => 'error-message']);
              $datasource->rollback();
            }
          }
          else {
            ...
            foreach ($productsPurchasedEarlier as $productPurchasedEarlier){
              $productPresent = false;
              $warningIfProblem="No era posible editar la entrada de suministros.<br/>";
              foreach ($this->request->data['Product'] as $product){
                if ($product['product_id']==$productPurchasedEarlier['StockMovement']['product_id']){
                  $productPresent=true;
                }
              ...
              if (!$productPresent){
                ...
                if ($stockItem['StockItem']['remaining_quantity']!=$stockItem['StockItem']['original_quantity']){
                  ...
                  $linkedProduct=$this->Product->find('first',array(
                    'conditions'=>array(
                      'Product.id'=>$productPurchasedEarlier['StockMovement']['product_id'],
                    ),
                  ));
                  $productName=$linkedProduct['Product']['name'];
                  $warningIfProblem.="Antes era presente el producto ".$productName.", y este producto ya ha estado ocupado en un orden de producción o una salida.";
                  $this->Session->setFlash($warningIfProblem, 'default',['class' => 'error-message']);
                }				
              }
            }
              ...
              foreach ($productsPurchasedEarlier as $purchaseStockMovement){
                ...
                if (count($stockMovementsForStockItemsInThisPurchase)>0){
                  ...
                  foreach ($stockMovementsForStockItemsInThisPurchase as $stockmovement){
                    ...
                    $this->Order->recursive=-1;
                    $linkedOrder=$this->Order->find('first',[
                      'conditions'=>['Order.id'=>$stockmovement['StockMovement']['order_id'],],
                    ]);
              ...
              foreach ($productsPurchasedEarlier as $purchaseStockMovement){
                ...
                if (count($productionMovementsForStockItemsInThisPurchase)>0){
                  ...
                  foreach ($productionMovementsForStockItemsInThisPurchase as $productionmovement){
                    ...
                    $this->ProductionRun->recursive=-1;
                    $linkedProductionRun=$this->ProductionRun->find('first',[
                      'conditions'=>['ProductionRun.id'=>$productionmovement['ProductionMovement']['production_run_id'],],
                    ]);
              ...
              try {
                ...
                foreach ($productsPurchasedEarlier as $productPurchasedEarlier){
                  ...
                  if (!$productPresent){
                    ...
                    $this->Product->recursive=-1;
                    $linkedProduct=$this->Product->find('first',[
                      'conditions'=>['Product.id'=>$productPurchasedEarlier['StockMovement']['product_id'],],
                    ]);
                    $productName=$linkedProduct['Product']['name'];
                ...
                $datasource->commit();
                $this->recordUserAction();
                $this->recordUserActivity($this->Session->read('User.username'),"Se editó la entrada de suministros # ".$this->request->data['Order']['order_code']);
                $this->Session->setFlash(__('Se editó la entrada de suministros.'),'default',['class' => 'success']);
                return $this->redirect(['action' => 'resumenEntradas']);
              }
              catch(Exception $e){
                $this->Session->setFlash(__('The purchase could not be saved. Please, try again.'), 'default',['class' => 'error-message']);
            ...
            'conditions'=>['StockMovement.product_quantity >'=>0,],
						'StockItem'=>[
							'ProductionMovement'=>[
								'conditions'=>['ProductionMovement.product_quantity >'=>0,],
								'ProductionRun',
							],
							'StockMovement'=>[
								'conditions'=>[
    ...
    $purchaseOrders=$this->PurchaseOrder->getConfirmedPurchaseOrders($warehouseId,1,$this->request->data['Order']['purchase_order_id']);
		$this->set(compact('purchaseOrders'));
    ...
    if ($this->request->is(['post', 'put']) && empty($this->request->data['refresh'])) {
      ...
      if (count($namedSales) > 0){
        $this->Session->setFlash(__('Ya existe una salida con el mismo código!  No se guardó la salida.'), 'default',['class' => 'error-message']);
      }
      elseif ($saleDateString > date('Y-m-d 23:59:59')){
        $this->Session->setFlash(__('La fecha de venta no puede estar en el futuro!  No se guardó la venta.'), 'default',['class' => 'error-message']);
      }
            ...
            try {
              ...
              foreach ($products as $product){
                ...
                if ($linkedProduct['Product']['product_type_id'] != PRODUCT_TYPE_SERVICE){
                  ...
                  if ($product_category_id==CATEGORY_PRODUCED){
                    $usedMaterials= $this->StockItem->getFinishedMaterialsForSale($productId,$production_result_code_id,$productQuantity,$raw_material_id,$saleDateString,$warehouseId);		
                  }
                  else {
                  ...
                  $stockMovementData['service_unit_cost']=$service_unit_cost;
                  $stockMovementData['service_total_cost']=$service_unit_cost*$productQuantity;
                  $stockMovementData['production_result_code_id']=$production_result_code_id;
      ...
      $this->StockMovement->virtualFields['service_total_cost']=0;
          ...
          $productInventory=$this->StockItem->getInventoryTotalQuantityAndValuePerProduct($stockMovement['StockMovement']['product_id'],$orderDate,$warehouseId,$stockMovement['StockItem']['raw_material_id']);
          $productCost=0;
    ...
    $this->set(compact('rawMaterials'));
      ...
      //pr($otherMaterialsInventory);
		...
		$this->set(compact('thirdParties', 'stockMovementTypes','products','finishedMaterialsInventory','otherMaterialsInventory','rawMaterialsInventory','productionResultCodes','rawMaterials','currencies','accountingCodes','bool_invoicetype_editable'));
            ...
            elseif (!$saleItemsOK) {
							...
							try	{
								...
								foreach ($products as $product){
									...
									// first prepare the materials that will be taken out of stock
									if ($this->Product->getProductCategoryId($productId)==CATEGORY_PRODUCED){
										$usedMaterials= $this->StockItem->getFinishedMaterialsForSale($productId,$production_result_code_id,$productQuantity,$raw_material_id,$remissionDateAsString,$warehouseId);
									}
									else {
