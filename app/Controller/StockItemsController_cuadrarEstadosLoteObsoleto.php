...
/******************** CUADRAR LOTES *******************/
    ...
    $finishedProductsInventory=$this->StockItem->getInventoryItems(PRODUCT_TYPE_BOTTLE,date('Y-m-d'),0,true);
    //pr($finishedProductsInventory);
      ...
      if ($productCategoryId==$categoryId){
        switch ($categoryId){
          ...
          case CATEGORY_CONSUMIBLE:
            $consumibleProductTypeIds=$this->ProductType->find('list',[
              'fields'=>['ProductType.id'],
              'conditions'=>['ProductType.product_category_id'=>CATEGORY_CONSUMIBLE]
            ]);
            break;
        ...
        switch ($categoryId){
          case CATEGORY_RAW:
            ...
            for ($i=0;$i<count($allRawStockItems);$i++){
              if (!empty($allRawStockItems[$i]['StockItem']['id'])){
                ...
                //pr($inputStockMovementTotalForStockItem);
                // edited 20150302
                //if (!empty($inputStockMovementTotalForStockItem[0]['total_product_quantity'])){
                  $allRawStockItems[$i]['StockItem']['total_moved_in']=(empty($inputStockMovementTotalForStockItem)?0:$inputStockMovementTotalForStockItem[0]['total_product_quantity']);
          ...
          case CATEGORY_CONSUMIBLE:
            // for consumibles
            //pr($consumibleProductTypeIds);
            $allConsumibleStockItems=$this->StockItem->find('all',array(
              'fields'=>array('StockItem.id, StockItem.original_quantity, StockItem.remaining_quantity, Product.name'),
              'conditions'=>array(
                'Product.product_type_id'=>$consumibleProductTypeIds,
                'StockItem.bool_active'=>true,
                'StockItem.stockitem_creation_date >'=>$startDate,
                //'StockItem.remaining_quantity !='=>0,
              ),
            ...
            for ($i=0;$i<count($allConsumibleStockItems);$i++){
              if (!empty($allConsumibleStockItems[$i]['StockItem']['id'])){
                $inputStockMovementTotalForStockItem=$this->StockMovement->find('first',array(
                  'fields'=>array('StockItem.id, SUM(StockMovement.product_quantity) AS total_product_quantity'),
                  'conditions'=>array(
                    'StockMovement.stockitem_id'=>$allConsumibleStockItems[$i]['StockItem']['id'],
                    'bool_input'=>true,
                  ),
                  'group'=>array('StockItem.id'),
                ));
                ...
                // edited 20150302
                //if (!empty($inputStockMovementTotalForStockItem[0]['total_product_quantity'])){
                  $allConsumibleStockItems[$i]['StockItem']['total_moved_in']=$inputStockMovementTotalForStockItem[0]['total_product_quantity'];
                  $productionMovementTotalForStockItem=$this->ProductionMovement->find('first',array(
                    'fields'=>array('StockItem.id, SUM(ProductionMovement.product_quantity) AS total_product_quantity'),
                    'conditions'=>array(
                      'ProductionMovement.stockitem_id'=>$allConsumibleStockItems[$i]['StockItem']['id'],
                      'bool_input'=>true,
                    ),
                    'group'=>array('StockItem.id'),
                  ));
                ...
                // edited 20150302
                //else {
                  //$allConsumibleStockItems[$i]['StockItem']['total_moved_in']=-1;
                //}
                if (!empty($productionMovementTotalForStockItem)){
                  $allConsumibleStockItems[$i]['StockItem']['total_used_in_production']=$productionMovementTotalForStockItem[0]['total_product_quantity'];
                }
                else {
                  $allConsumibleStockItems[$i]['StockItem']['total_used_in_production']=0;
                }
                $stockMovementTotalForStockItem=$this->StockMovement->find('first',array(
                  'fields'=>array('StockItem.id, SUM(StockMovement.product_quantity) AS total_product_quantity'),
                  'conditions'=>array(
                ...
                if (!empty($stockMovementTotalForStockItem)){
                  $allConsumibleStockItems[$i]['StockItem']['total_moved_out']=$stockMovementTotalForStockItem[0]['total_product_quantity'];
                }
                else {
                  $allConsumibleStockItems[$i]['StockItem']['total_moved_out']=0;
                }
                $lastStockItemLog=$this->StockItemLog->find('first',array(
                  'fields'=>array('StockItemLog.id, StockItemLog.product_quantity'),
                  'conditions'=>array(
                    'StockItemLog.stockitem_id'=>$allConsumibleStockItems[$i]['StockItem']['id'],
                  ),
                  'order'=>array('StockItemLog.id DESC,StockItemLog.stockitem_date DESC'),
                ));
                
                // edited 20150302
                //$allConsumibleStockItems[$i]['StockItem']['latest_log_quantity']=(!empty($lastStockItemLog['StockItemLog']['product_quantity'])?$lastStockItemLog['StockItemLog']['product_quantity']:"N/A");
                //$allConsumibleStockItems[$i]['StockItem']['latest_log_id']=(!empty($lastStockItemLog['StockItemLog']['id'])?$lastStockItemLog['StockItemLog']['id']:"N/A");
                $allConsumibleStockItems[$i]['StockItem']['latest_log_quantity']=$lastStockItemLog['StockItemLog']['product_quantity'];
                $allConsumibleStockItems[$i]['StockItem']['latest_log_id']=$lastStockItemLog['StockItemLog']['id'];
              }
            }
            //pr($allConsumibleStockItems);
            break;
          case CATEGORY_PRODUCED:
            ...
            if (!empty($finishedProductId)){
              $conditions[]=['Product.id'=>$finishedProductId];
            }
            //pr($conditions);
            $allFinishedStockItems=$this->StockItem->find('all',[
              'fields'=>['StockItem.id, StockItem.original_quantity, StockItem.remaining_quantity, ProductionResultCode.code, RawMaterial.name, Product.name'],
              'conditions'=>$conditions,
            ...
            for ($i=0;$i<count($allFinishedStockItems);$i++){
              ...
              $adjustmentMovementTotalForStockItem=$this->StockMovement->find('first',[
              ...
              if (!empty($adjustmentMovementTotalForStockItem[0])){
                $allFinishedStockItems[$i]['StockItem']['total_produced_in_production']+=$adjustmentMovementTotalForStockItem[0]['total_product_quantity'];
              }
    ...
    $this->set(compact('allRawStockItems','allConsumibleStockItems','allFinishedStockItems','allOtherStockItems'));
	}
